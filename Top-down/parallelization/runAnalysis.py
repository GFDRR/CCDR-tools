...
def run_analysis(run_country):
    ...
    if analysis_type == "Classes":
        ...
        seq = np.all([True if class_edges[i] < class_edges[i + 1] else False for i in range(0, len(class_edges) - 1)])
        # If error
        if not seq:
    ...
    stats_parallel = p.map(zonal_stats_parallel, zip(chunks(adm_data["geometry"], cores), it.repeat(exp_ras), it.repeat('sum')))
